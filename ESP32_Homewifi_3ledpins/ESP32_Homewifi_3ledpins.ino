...
const char* ssid = "Prof. Hussein"; //"YOUR_WIFI_NAME";   // 🔴 CHANGE: your lab WiFi name
const char* password = "1q2w3e4r5t"; //"YOUR_WIFI_PASSWORD";       // 🔴 CHANGE: your WiFi password

// const char* ssid = "roboticsclub_iptime01"; //"YOUR_WIFI_NAME";             // 🔴 CHANGE: your home WiFi name
// const char* password = "robot1234"; //"YOUR_WIFI_PASSWORD";

WiFiServer server(80);          // HTTP server runs on port 80
WebSocketsServer webSocket = WebSocketsServer(81); // WebSocket on port 81

// ---------------- Pins ----------------
const int L1 = 2;           // GPIO 2 → LED output (PWM)
const int L2 = 4;  
const int L3 = 17;  
...
<h2>LEDs Brightness Control</h2>

<div class="container">
  <div class='box'>
    <h2>LED1</h2>
    <div class='value' id='ledValue1'>0%</div>
    <input type='range' min='0' max='100' value='0'
    oninput="sendLED(this.value,'ledValue1')">
  </div>

  <div class='box'>
    <h2>LED2</h2>
    <div class='value' id='ledValue2'>0%</div>
    <input type='range' min='0' max='100' value='0'
    oninput="sendLED(this.value,'ledValue2')">
  </div>

  <div class='box'>
    <h2>LED3</h2>
    <div class='value' id='ledValue3'>0%</div>
    <input type='range' min='0' max='100' value='0'
    oninput="sendLED(this.value,'ledValue3')">
  </div>
</div>
...
function sendLED(val, valID){
document.getElementById(valID).innerHTML = val + '%';
socket.send(valID + ":" + val); // Send value via WebSocket
}
...
pinMode(L1, OUTPUT);
pinMode(L2, OUTPUT);              // Set LED pin as output
pinMode(L3, OUTPUT);              // Set LED pin as output

analogWrite(L1, 0);               // Initialize LED OFF
analogWrite(L2, 0);
analogWrite(L3, 0);
...
if (MDNS.begin("esp32robot11")) {
  Serial.println("mDNS started → http://esp32robot11.local");
...
{                // If text message received
  ...
  if (index != -1) 
  {
    String msgID = msg.substring(0, index);

    if (msgID == "ledValue1") {
      ledPin = L1;
    } else if (msgID == "ledValue2") {
      ledPin = L2;
    } else if (msgID == "ledValue3") {
      ledPin = L3;
    }

    int val = msg.substring(index+1).toInt();                  // Convert to integer
